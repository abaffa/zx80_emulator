...
static void z80_exec_extended_DDCB(struct z80* z80, unsigned char* memory)
{
    unsigned char I; //register
    unsigned short J; //register

    J = z80->registers.IX + OpZ80(z80, memory, z80->registers.PC);
...
static void z80_exec_extended_DD(struct z80* z80, unsigned char* memory)
{
  unsigned char I; //register
  unsigned short J; //register
...
//  #define XX IX
  ...
  switch(I)
  {
    ...
    case ADD_xHL:       debug_opcode(z80, (char *)"ADD_xIX+BYTE", (char *)"Adds the value pointed by (IX) plus BYTE to A");
        I = RdZ80(memory, z80->registers.IX + OpZ80(z80, memory, z80->registers.PC));
        M_ADD(z80, I); break;
    ...
    case INC_xHL:       debug_opcode(z80, (char *)"INC_xIX+BYTE", (char *)"Adds one to the memory location pointed to by IX plus BYTE.");
        I = RdZ80(memory, z80->registers.IX + RdZ80(memory, z80->registers.PC));
        M_INC_L(z80, I);
        WrZ80(memory,  z80->registers.IX + OpZ80(z80, memory, z80->registers.PC), I);
        break;
    ...
    case PUSH_HL:       debug_opcode(z80, (char *)"PUSH_IX", (char *)"SP is decremented and IXh is stored into the memory location pointed to by SP. SP is decremented again and IXl is stored pointed to by SP"); 
        M_PUSH(z80, memory, IX);break;

                    
      case LD_HL_WORD:    debug_opcode_reg_word(z80, memory, (char *)"LD_IX_WORD", (char *)"Loads ** into IX"); 
          M_LDWORD(z80, memory, z80->registers.IX);break;
      case LD_xWORD_HL:    debug_opcode_reg_word(z80, memory, (char *)"LD_xWORD_IX", (char *)"Stores IX into the memory location pointed to by WORD");
        SET_LSB(J, OpZ80(z80, memory, z80->registers.PC));
        SET_MSB(J, OpZ80(z80, memory, z80->registers.PC));
        WrZ80(memory, J, LSB(z80->registers.IX));
        J += 1;
        WrZ80(memory, J, MSB(z80->registers.IX));
        break;
      case LD_H_BYTE:     debug_opcode_reg_byte(z80, memory, (char *)"LD_IXh_BYTE", (char *)"Load BYTE into IXh");
          SET_MSB(z80->registers.IX, OpZ80(z80, memory, z80->registers.PC));
          break;
      case LD_HL_xWORD:    debug_opcode_reg_word(z80, memory, (char *)"LD_IX_xWORD", (char *)"Load value pointed to by WORD into IX");
        SET_LSB(J, OpZ80(z80, memory, z80->registers.PC));
        SET_MSB(J, OpZ80(z80, memory, z80->registers.PC));
        SET_LSB(z80->registers.IX, RdZ80(memory, J));
        J += 1;
        SET_MSB(z80->registers.IX, RdZ80(memory, J));
        break;
      case LD_L_BYTE:     debug_opcode_reg_byte(z80, memory, (char *)"LD_IXl_BYTE", (char *)"Load BYTE into IXl");
          SET_LSB(z80->registers.IX, OpZ80(z80, memory, z80->registers.PC));
          break;

      case LD_xHL_BYTE:   debug_opcode_reg_byte(z80, memory, (char *)"LD_xIXl_BYTE", (char *)"Stores BYTE to the memory location pointed to by IX plus BYTE");
          J = z80->registers.IX + OpZ80(z80, memory, z80->registers.PC);
      ...
      case LD_B_xHL:      debug_opcode(z80, (char *)"LD_B_xIX", (char *)"The contents of IX plus BYTE are loaded into B");
          SET_MSB(z80->registers.BC, RdZ80(memory, z80->registers.IX + OpZ80(z80, memory, z80->registers.PC)));
          break;
      ...
      case LD_C_xHL:      debug_opcode(z80, (char *)"LD_C_xIX", (char *)"The contents of IX plus BYTE are loaded into C");
          SET_LSB(z80->registers.BC, RdZ80(memory, z80->registers.IX + OpZ80(z80, memory, z80->registers.PC)));
          break;
      ...
      case LD_D_xHL:      debug_opcode(z80, (char *)"LD_D_xIX", (char *)"The contents of IX plus BYTE are loaded into D");
          SET_MSB(z80->registers.DE, RdZ80(memory, z80->registers.IX + OpZ80(z80, memory, z80->registers.PC)));
          break;
      ...
      case LD_E_xHL:      debug_opcode(z80, (char *)"LD_E_xIX", (char *)"The contents of IX plus BYTE are loaded into E");
          SET_LSB(z80->registers.DE, RdZ80(memory, z80->registers.IX + OpZ80(z80, memory, z80->registers.PC)));
          break;
      ...
      case LD_H_xHL:      debug_opcode(z80, (char *)"LD_H_IXL", (char *)"Loads the value pointed to IX plus BYTE into H");
          SET_MSB(z80->registers.HL, RdZ80(memory, z80->registers.IX + OpZ80(z80, memory, z80->registers.PC)));
          break;
      ...
      case LD_L_xHL:        debug_opcode(z80, (char *)"LD_L_xIX", (char *)"Loads the value pointed to IX plus BYTE into L");
          SET_LSB(z80->registers.HL, RdZ80(memory, z80->registers.IX + OpZ80(z80, memory, z80->registers.PC)));
          break;
      ...
      case LD_A_xHL:        debug_opcode(z80, (char *)"LD_L_xIX", (char *)"Loads the value pointed to IX plus BYTE into A");
          SET_MSB(z80->registers.AF, RdZ80(memory, z80->registers.IX+ OpZ80(z80, memory, z80->registers.PC)));
          break;
    ...
    case PFX_CB:
      z80_exec_extended_DDCB(z80, memory);break;
    default:{
      char str2[20];
      sprintf(str2,"unknown 0x%x",opcode);
      debug_opcode(z80, str2, (char*)"");
